...
async def enviar_comando_agente(id_vps, acao, agent_url, agent_token):
    headers = {"X-API-Key": agent_token}
    url_limpa = agent_url.rstrip('/')
    url_comando = f"{url_limpa}/control/{acao}"
    async with httpx.AsyncClient(timeout=15.0, verify=False, follow_redirects=True) as client:
        try:
            resp = await client.post(url_comando, json={"vps_id": id_vps}, headers=headers)
...
def verificar_admin(credentials: HTTPBasicCredentials = Depends(security)):
    ...
    if not (correct_username and correct_password):
        raise HTTPException(status_code=status.HTTP_401_UNAUTHORIZED, detail="Acesso Restrito", headers={"WWW-Authenticate": "Basic"})
...
def init_db():
    ...
    conn.execute('''CREATE TABLE IF NOT EXISTS nodes (
        id INTEGER PRIMARY KEY AUTOINCREMENT, nome TEXT, url_agente TEXT, token_agente TEXT, 
        arquitetura TEXT, preco_usd REAL, limite_vps INTEGER, ativo INTEGER DEFAULT 1)''')
    conn.execute('''CREATE TABLE IF NOT EXISTS tickets (
        id INTEGER PRIMARY KEY AUTOINCREMENT, email TEXT, assunto TEXT, mensagem TEXT, resposta TEXT,
        status TEXT DEFAULT 'OPEN', data_criacao DATETIME DEFAULT CURRENT_TIMESTAMP)''')
    
    cursor = conn.cursor()
    cursor.execute("PRAGMA table_info(users)")
    if 'reset_token' not in [c[1] for c in cursor.fetchall()]: conn.execute("ALTER TABLE users ADD COLUMN reset_token TEXT")

    cursor.execute("PRAGMA table_info(vps)")
    colunas_vps = [c[1] for c in cursor.fetchall()]
    ...
    colunas_nodes = [c[1] for c in cursor.fetchall()]
...
def alocar_ips_disponiveis(node_id):
    conn = sqlite3.connect("db.sqlite")
    node = conn.execute("SELECT ipv4_base, ipv4_cidr, ipv6_base, ipv6_cidr, ipv4_gw, ipv6_gw, limite_vps FROM nodes WHERE id = ?", (node_id,)).fetchone()
    ...
    for ip_v4, ip_v6 in ips_em_uso:
        if ip_v4:
            try: v4_usados.append(int(ip_v4.split('/')[0].split('.')[-1]))
            except: pass
        if ip_v6:
            try: v6_usados.append(int(ipaddress.IPv6Address(ip_v6.split('/')[0])))
            except: pass

    novo_v4_sufixo = next((i for i in range(1, limite + 5) if i not in v4_usados), None)
            
    try: cidr_v6 = int(ipv6_cidr)
    except: cidr_v6 = 128
    
    step_v6 = 2 ** (128 - cidr_v6) if cidr_v6 < 128 else 1
    ipv6_base_clean = ipv6_base if not ipv6_base.endswith(":") else ipv6_base + "0"
    base_v6_int = int(ipaddress.IPv6Address(ipv6_base_clean))
    ...
    return {
        "ipv4": f"{ipv4_base}{novo_v4_sufixo}/{ipv4_cidr}" if ipv4_base else "", "ipv4_gw": ipv4_gw,
        "ipv6": f"{novo_v6_str}/{ipv6_cidr}" if ipv6_base else "", "ipv6_gw": ipv6_gw
    }
...
async def vps_page(request: Request):
    ...
    nodes = [{"nome": n[0], "arquitetura": n[1], "preco_usd": n[2], "preco_pol": f"{calcular_pol_necessario(n[2]):.6f}", "ram_mb": n[3], "swap_mb": n[4], "disk_mb": n[5], "cpu_fraction": n[6], "descricao_hardware": n[7]} for n in nodes_db]
    conn.close()
...
async def login_page(request: Request): return templates.TemplateResponse("login.html", {"request": request, "msg": request.query_params.get("msg", ""), "mode": "login"})
...
async def forgot_page(request: Request): return templates.TemplateResponse("login.html", {"request": request, "msg": request.query_params.get("msg", ""), "mode": "forgot"})
...
async def reset_post(token: str, pw: str = Form(...)):
    conn = sqlite3.connect("db.sqlite")
    if conn.execute("SELECT email FROM users WHERE reset_token=?", (token,)).fetchone():
...
async def dash(request: Request):
    ...
    conn = sqlite3.connect("db.sqlite")
    pedidos_db = conn.execute("SELECT v.id, v.carteira, v.status, n.preco_usd, v.validade, n.preco_renew_usd, n.preco_ano_usd, n.nome, n.arquitetura FROM vps v JOIN nodes n ON v.node_id = n.id WHERE v.email=? AND v.status NOT IN ('DELETED', 'TERMINATED')", (email,)).fetchall()
    pedidos = [{"id": p[0], "carteira": p[1], "status": p[2], "preco_usd": p[3], "preco_pol": f"{calcular_pol_necessario(p[3]):.6f}", "validade": p[4], "renew_usd": p[5], "renew_pol": f"{calcular_pol_necessario(p[5]):.6f}", "ano_usd": p[6], "ano_pol": f"{calcular_pol_necessario(p[6]):.6f}", "node_nome": p[7], "arquitetura": p[8]} for p in pedidos_db]
    ...
    for n in nodes_db:
        ...
        nodes_disponiveis.append({"id": n[0], "nome": n[1], "arquitetura": n[2], "preco_usd": n[3], "preco_pol": f"{calcular_pol_necessario(n[3]):.6f}", "ram_mb": n[4], "swap_mb": n[5], "disk_mb": n[6], "cpu_fraction": n[7], "uso": uso, "limite": n[8], "sold_out": uso >= n[8], "descricao_hardware": n[9], "cpu_core": n[10]})
...
async def comprar(request: Request, bg_tasks: BackgroundTasks, node_id: int = Form(...)):
    ...
    conn.execute("INSERT INTO vps (id, email, carteira, chave_privada, status, preco_pol, node_id) VALUES (?, ?, ?, ?, ?, ?, ?)", (id_pedido, email, endereco, chave_privada, "PENDING PAYMENT", float(f"{calcular_pol_necessario(node[0]):.6f}"), node_id))
    conn.commit()
    conn.close()
...
async def verificar_pagamento(id_vps: str, request: Request, bg_tasks: BackgroundTasks):
    ...
    if not email: return RedirectResponse("/login")
    conn = sqlite3.connect("db.sqlite")
    vps = conn.execute("SELECT v.carteira, v.chave_privada, v.status, v.validade, n.preco_usd, n.preco_renew_usd, n.preco_ano_usd FROM vps v JOIN nodes n ON v.node_id = n.id WHERE v.id=? AND v.email=?", (id_vps, email)).fetchone()
    if not vps or vps[2] not in ['PENDING PAYMENT', 'ATIVA', 'SUSPENDED']: return RedirectResponse(url="/dash", status_code=303)
    
    endereco, chave_privada, status, validade, preco_usd, preco_renew_usd, preco_ano_usd = vps
    dias_add = 365 if verificar_pagamento_pol(endereco, float(f"{calcular_pol_necessario(preco_ano_usd):.6f}")) else 30 if verificar_pagamento_pol(endereco, float(f"{calcular_pol_necessario(preco_renew_usd):.6f}")) else 30 if (status == 'PENDING PAYMENT' and verificar_pagamento_pol(endereco, float(f"{calcular_pol_necessario(preco_usd):.6f}"))) else 0
    
    if dias_add > 0:
        conn.execute(f"UPDATE vps SET status = 'ATIVA', validade = CASE WHEN datetime(validade) > datetime('now') THEN datetime(validade, '+{dias_add} days') ELSE datetime('now', '+{dias_add} days') END WHERE id = ?" if status in ['ATIVA', 'SUSPENDED'] and validade else f"UPDATE vps SET status = 'ATIVA', validade = datetime('now', '+{dias_add} days') WHERE id = ?", (id_vps,))
        conn.commit()
        registrar_log("PAGO", f"VPS {id_vps} renovada ({dias_add} dias).", "SUCCESS", email)
        if MINHA_CARTEIRA_PRINCIPAL: varrer_carteira(endereco, chave_privada, MINHA_CARTEIRA_PRINCIPAL)
...
async def processar_ativacao_apos_pagamento(id_vps, email):
    ...
    resultado = await chamar_agent_banana_pi(id_vps, agent_url, agent_token, ram_mb=ram, swap_mb=swap, disk_mb=disk, cpu_fraction=cpu, cpu_core=cpu_core, ipv4=rede["ipv4"], ipv4_gw=rede["ipv4_gw"], ipv6=rede["ipv6"], ipv6_gw=rede["ipv6_gw"], deploy_script=deploy_script)
...
async def apagar_vps(id_vps: str, request: Request, bg_tasks: BackgroundTasks):
    ...
    conn.commit()
    conn.close()
    if agent_url: bg_tasks.add_task(enviar_comando_agente, id_vps, "delete", agent_url, agent_token)
...
async def painel_ops(request: Request, admin: str = Depends(verificar_admin)):
    ...
    vps_geral = conn.execute("SELECT v.id, v.email, v.status, v.node_id, v.validade, n.nome, n.arquitetura FROM vps v JOIN nodes n ON v.node_id = n.id").fetchall()
    ...
    nodes = [n + (conn.execute("SELECT COUNT(*) FROM vps WHERE node_id=? AND status NOT IN ('DELETED', 'TERMINATED')", (n[0],)).fetchone()[0],) for n in nodes_db]
    ...
    return templates.TemplateResponse("ops.html", {"request": request, "usuarios": usuarios, "vps": vps_geral, "logs": logs, "nodes": nodes, "gas": gas, "tickets": tickets_db, "msg": msg})

@app.post("/ops/add_node")
async def ops_add_node(request: Request, nome: str = Form(...), url_agente: str = Form(...), token_agente: str = Form(...), arquitetura: str = Form(...), preco_usd: float = Form(...), preco_renew_usd: float = Form(...), preco_ano_usd: float = Form(...), limite: int = Form(...), ram_mb: int = Form(...), swap_mb: int = Form(...), disk_mb: int = Form(...), cpu_fraction: str = Form(...), descricao_hardware: str = Form(...), cpu_core: str = Form(...), ordem: int = Form(0), ipv4_base: str = Form(...), ipv4_cidr: str = Form(...), ipv4_gw: str = Form(...), ipv6_base: str = Form(...), ipv6_cidr: str = Form(...), ipv6_gw: str = Form(...), deploy_script: str = Form("create_vps.sh"), admin: str = Depends(verificar_admin)):
    conn = sqlite3.connect("db.sqlite")
    conn.execute("INSERT INTO nodes (nome, url_agente, token_agente, arquitetura, preco_usd, preco_renew_usd, preco_ano_usd, limite_vps, ram_mb, swap_mb, disk_mb, cpu_fraction, descricao_hardware, cpu_core, ordem, ipv4_base, ipv4_cidr, ipv4_gw, ipv6_base, ipv6_cidr, ipv6_gw, deploy_script) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)", (nome, url_agente, token_agente, arquitetura, preco_usd, preco_renew_usd, preco_ano_usd, limite, ram_mb, swap_mb, disk_mb, cpu_fraction, descricao_hardware, cpu_core, ordem, ipv4_base, ipv4_cidr, ipv4_gw, ipv6_base, ipv6_cidr, ipv6_gw, deploy_script))
    conn.commit()
    conn.close()
    return RedirectResponse(url="/ops?msg=Node+Adicionado+com+Sucesso", status_code=303)

@app.post("/ops/edit_node/{node_id}")
async def ops_edit_node(node_id: int, request: Request, nome: str = Form(...), url_agente: str = Form(...), token_agente: str = Form(...), arquitetura: str = Form(...), preco_usd: float = Form(...), preco_renew_usd: float = Form(...), preco_ano_usd: float = Form(...), limite: int = Form(...), ram_mb: int = Form(...), swap_mb: int = Form(...), disk_mb: int = Form(...), cpu_fraction: str = Form(...), descricao_hardware: str = Form(...), cpu_core: str = Form(...), ordem: int = Form(0), ipv4_base: str = Form(...), ipv4_cidr: str = Form(...), ipv4_gw: str = Form(...), ipv6_base: str = Form(...), ipv6_cidr: str = Form(...), ipv6_gw: str = Form(...), deploy_script: str = Form("create_vps.sh"), admin: str = Depends(verificar_admin)):
    conn = sqlite3.connect("db.sqlite")
    conn.execute("UPDATE nodes SET nome=?, url_agente=?, token_agente=?, arquitetura=?, preco_usd=?, preco_renew_usd=?, preco_ano_usd=?, limite_vps=?, ram_mb=?, swap_mb=?, disk_mb=?, cpu_fraction=?, descricao_hardware=?, cpu_core=?, ordem=?, ipv4_base=?, ipv4_cidr=?, ipv4_gw=?, ipv6_base=?, ipv6_cidr=?, ipv6_gw=?, deploy_script=? WHERE id=?", (nome, url_agente, token_agente, arquitetura, preco_usd, preco_renew_usd, preco_ano_usd, limite, ram_mb, swap_mb, disk_mb, cpu_fraction, descricao_hardware, cpu_core, ordem, ipv4_base, ipv4_cidr, ipv4_gw, ipv6_base, ipv6_cidr, ipv6_gw, deploy_script, node_id))
    conn.commit()
    conn.close()
...
async def processar_ativacao_manual(id_vps, email):
    ...
    conn.execute("UPDATE vps SET ipv4 = ?, ipv6 = ? WHERE id = ?", (rede["ipv4"], rede["ipv6"], id_vps))
    conn.commit()
    res = await chamar_agent_banana_pi(id_vps, agent_url, agent_token, ram_mb=ram, swap_mb=swap, disk_mb=disk, cpu_fraction=cpu, cpu_core=cpu_core, ipv4=rede["ipv4"], ipv4_gw=rede["ipv4_gw"], ipv6=rede["ipv6"], ipv6_gw=rede["ipv6_gw"], deploy_script=deploy_script)
    if res.get("sucesso"):
...
if __name__ == "__main__":
    import uvicorn
    uvicorn.run("main:app", host="127.0.0.1", port=8000, workers=1, reload=False, access_log=False)
